...
function HaloRing({ radius = 1.6, tube = 0.04, color = '#D4AC70', rotation = [0, 0, 0], speed = 0.3 }) {
  const ref = useRef();
  useFrame((_, dt) => {
    if (ref.current) {
...
function Scene({ reduced }) {
  return (
    <>
      ...
      <pointLight position={[-3, -2, 2]} intensity={1.2} color={'#F3DBD2'} />
      <pointLight position={[3, -3, -2]} intensity={1.0} color={'#EFD9B1'} />
